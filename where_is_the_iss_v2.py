...
parser.add_argument(
	'--db', '--database', type=str, required=True, dest='database',
	help='Database name'
)
parser.add_argument(
	'--collection', type=str, required=True, dest='collection',
...
def call_api_and_store_result_in_file(url, filename, collection, preprocess):
	header = [
	...
	with lock:
		collection.insert_one(iss_json)
		print('Inserted document to the collection')
		write_row_to_file(iss_json, filename, header)


def request_until_stopped(interval_in_secs, filename, collection, preprocess):
	...
	try:
		...
		while True:
			threading.Thread(target=call_api_and_store_result_in_file, args=(iss_api_endpoint_url, filename, collection, preprocess)).start()
...
if __name__ == '__main__':
	...
	# Request
	request_until_stopped(interval_in_secs, args.output_file, collection, args.preprocess)
